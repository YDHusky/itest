...
class ItestKimi(Kimi):
    main_sysprompt = """你需要以一个大学生的身份,你正在做一篇试卷,需要以 JSON 格式返回数据。请确保你的响应是有效的 JSON 对象。你需要严格按照以下格式完成题目,有写作,翻译，选择，排序题,写作和翻译需要返回文章内容,选择题需要以列表的形式按照顺序返回答案, 需要完整输出json,不要断开"""
    write_sysprompt = """
    输入样例: 
